...
//    }
    ...
    public List<Loft> getClub(double yardage, double lie) {
        // iterate through loftList as long as distance is decreasing to find closest match
        // as soon as the difference between club distances starts to increase abort search
        int numOfResults = 5;
        List<Loft> results;
        Loft[] temp = new Loft[numOfResults];
        double diff = 100000;
        double currentDiff;
        Loft bestLoft = null;
        int i = 0;
        ...
        results = new ArrayList<Loft>(Arrays.asList(temp));
        Collections.sort(results, new LoftDifferenceComparator());
...
//        Loft[] chosenShots = bag.getClub(shotDist, lieModifier);
//        for (Loft club : chosenShots)
